...
class GenericNNetWrapper(NeuralNet):
	def __init__(self, game, nn_args, use_exchange=False):
		self.args = nn_args
		self.device = {
			'training' : 'cuda' if torch.cuda.is_available() else 'cpu',
			'inference': 'onnx',
			'just_loaded': 'cpu',
		}
		self.current_mode = 'cuda'
		self.init_nnet(game, nn_args, use_exchange=False)
